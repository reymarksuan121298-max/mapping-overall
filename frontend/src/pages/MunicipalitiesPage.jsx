...
export default function MunicipalitiesPage() {
  ...
  const fetchMunicipalities = async () => {
    ...
    try {
      ...
      const enrichedData = (data || []).map(m => ({
        ...m,
        employeeCount: m.employees ? m.employees.length : 0
      }));
  ...
  const openEditModal = (municipality) => {
    setModalMode('edit');
    setFormData({ name: municipality.name });
  ...
  const handleDelete = (id) => {
    setConfirmState({
      isOpen: true,
      message: 'Are you sure you want to delete this municipality? All linked data may be affected.',
      onConfirm: async () => {
        ...
        try {
          ...
          setAlertState({ isOpen: true, message: 'Failed to delete municipality.', type: 'error' });
        }
      }
    });
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      console.error('Error saving municipality:', err.message);
      let errorMsg = 'Failed to save municipality.';
      if (err.code === '23505' || err.message.includes('duplicate key') || err.message.includes('unique constraint')) {
        errorMsg = 'A municipality with this name already exists.';
      }
        ...
        <header className="mb-8 flex items-end justify-between">
          <div>
            <h1 className="text-3xl font-black bg-clip-text text-transparent bg-gradient-to-r from-teal-400 to-indigo-500 flex items-center gap-3">
              <LandPlot className="text-teal-500" size={32} />
              Municipality Management
            </h1>
            <p className="text-slate-400 mt-2 font-medium">Manage municipality zones and monitor geographical workforce distribution</p>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-800/80 text-slate-400 text-xs uppercase tracking-widest">
                  <th className="px-6 py-4 font-bold border-b border-slate-700/50">Municipality Name</th>
                  ...
                  filteredMunicipalities.map(m => (
                    <tr key={m.id} className="hover:bg-slate-800/40 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="font-bold text-slate-200 text-base">{m.name}</div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2 opacity-100 transition-opacity">
                          <button onClick={() => openEditModal(m)} className="p-2 hover:bg-slate-700 rounded-lg text-slate-400 hover:text-blue-400 transition-colors">
                            <Edit2 size={16} />
                          </button>
                          <button onClick={() => handleDelete(m.id)} className="p-2 hover:bg-slate-700 rounded-lg text-slate-400 hover:text-rose-400 transition-colors">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </td>
            ...
            >
              <div className="px-6 py-4 border-b border-slate-700 flex items-center justify-between bg-slate-800/50">
                <h3 className="text-lg font-bold text-slate-200 flex items-center gap-2">
                  ...
                  {modalMode === 'add' ? 'Add Municipality' : 'Edit Municipality'}
                </h3>
              ...
              <form onSubmit={handleSave} className="p-6 space-y-4">
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Municipality Name</label>
                  <input
                    type="text"
                    required
                    value={formData.name}
