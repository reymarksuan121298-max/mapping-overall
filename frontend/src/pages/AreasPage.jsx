...
export default function AreasPage() {
  ...
  const fetchAreas = async () => {
    ...
    try {
      ...
      const enrichedData = (data || []).map(area => ({
        ...area,
        employeeCount: area.employees ? area.employees.length : 0
      }));
      
      setAreas(enrichedData);
    } catch (error) {
      ...
      <div className="w-full relative">
        <header className="mb-8 flex items-end justify-between">
          <div>
            ...
            <p className="text-slate-400 mt-2 font-medium">Manage operational areas and monitor their workforce</p>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-800/80 text-slate-400 text-xs uppercase tracking-widest">
                  <th className="px-6 py-4 font-bold border-b border-slate-700/50">Area Name</th>
              ...
              <tbody className="divide-y divide-slate-800/50">
                {loading ? (
                  ...
                  filteredAreas.map(area => (
                    <tr key={area.id} className="hover:bg-slate-800/40 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="font-bold text-slate-200 text-base">{area.name}</div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2 opacity-100 transition-opacity">
                          <button onClick={() => openEditModal(area)} className="p-2 hover:bg-slate-700 rounded-lg text-slate-400 hover:text-blue-400 transition-colors">
                            <Edit2 size={16} />
                          </button>
                          <button onClick={() => handleDelete(area.id)} className="p-2 hover:bg-slate-700 rounded-lg text-slate-400 hover:text-rose-400 transition-colors">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </td>
            ...
            >
              <div className="px-6 py-4 border-b border-slate-700 flex items-center justify-between bg-slate-800/50">
                <h3 className="text-lg font-bold text-slate-200 flex items-center gap-2">
                  ...
                  {modalMode === 'add' ? 'Add Area' : 'Edit Area'}
                </h3>
              ...
              <form onSubmit={handleSave} className="p-6 space-y-4">
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Area Name</label>
                  <input
                    type="text"
                    required
                    value={formData.name}
